...
export class CreateWarehouseComponent implements OnInit {
  ...
  async onSubmit() {
    ...
    if (answer.status == 201) {
      const updatedWarehouse = await this.warehouseService.getWarehouse(this.formCreateWarehouse.value.Id);
      this.warehouseService.createWarehouseProlog(updatedWarehouse);
      this.warehouseService.createCityProlog(updatedWarehouse);
    } else
